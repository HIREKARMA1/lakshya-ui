import type { ProviderProfile, SeekerProfile } from "@/types/auth";
import type { Job } from "@/types/job";

export function buildSeekerAddress(profile?: SeekerProfile | null): string {
  if (!profile) return "";
  const exact = profile.exact_location?.trim();
...
export function buildProviderAddress(profile?: ProviderProfile | null): string {
  if (!profile) return "";
  const searchAddr = profile.search_addr?.trim();
  if (searchAddr) return searchAddr;
  const parts = [profile.pincode, profile.area, profile.locality, profile.city, profile.district, profile.state]
    .map((p) => (p ?? "").trim())
    .filter(Boolean);
  if (!parts.length) return "";
  if (!parts.join(" ").toLowerCase().includes("india")) parts.push("India");
