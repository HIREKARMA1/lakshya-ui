...
import { MapPin, Users, Zap, UserCheck, LayoutGrid, Clock } from "lucide-react";
import Image from "next/image";

interface Fact { k: string; v: string }

const FACT_ICONS = [MapPin, Users, Zap];
const STRIP_ICONS = [UserCheck, LayoutGrid, Clock];

export function SeekersHero({
  kicker,
  title,
  subtitle,
  facts,
  ...
  return (
    <section className="relative isolate">
      ...
      >
        ...
        <div className="grid items-center gap-4 px-5 py-4 sm:grid-cols-[1fr_auto] sm:px-8 sm:py-5">
          ...
          <div className="hidden shrink-0 sm:block">
            <Image
              src="/assets/seekers-illustration.png"
              alt=""
              width={180}
              height={220}
      ...
      <div className="mt-3 grid grid-cols-1 overflow-hidden rounded-xl border border-line bg-white shadow-sm sm:grid-cols-3">
        {stripArr.map((s, i) => {
          const Icon = STRIP_ICONS[i] ?? UserCheck;
          return (
            <div
              key={s.k}
