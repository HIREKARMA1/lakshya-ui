...
type HelpSupportContentProps = {
  /** i18n key prefix, e.g. `dashboard.help` or `providerDashboard.help` */
  i18nKey: string;
};

export function HelpSupportContent({ i18nKey }: HelpSupportContentProps) {
  const { t } = useTranslation();
  const faqs = readList<FaqItem>(t(`${i18nKey}.faqs`, { returnObjects: true }));
  const guides = readList<GuideItem>(t(`${i18nKey}.guides`, { returnObjects: true }));
  const safetyItems = readList<string>(t(`${i18nKey}.safetyItems`, { returnObjects: true }));
  ...
  return (
    <div className="w-full space-y-6">
      <div>
        <h1 className="font-display text-2xl font-extrabold text-ink">{t(`${i18nKey}.title`)}</h1>
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">{t(`${i18nKey}.subtitle`)}</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-12 lg:items-start">
        <div className="space-y-6 lg:col-span-8">
          <section className={cardCls}>
            <h2 className="font-display text-lg font-bold text-ink">{t(`${i18nKey}.faqTitle`)}</h2>
            <ul className="mt-4 grid gap-4 sm:grid-cols-2">
              {faqs.map((faq) => (
                <li key={faq.q} className="rounded-lg border border-line/80 bg-soft/15 p-4 sm:p-5">
          ...
          <section className={cardCls}>
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="font-display text-lg font-bold text-ink">{t(`${i18nKey}.guidesTitle`)}</h2>
            </div>
        ...
        <aside className="space-y-6 lg:sticky lg:top-4 lg:col-span-4 lg:self-start">
          ...
          <section className={cardCls}>
            <div className="mb-2 flex items-center gap-2">
              ...
              <h2 className="font-display text-lg font-bold text-ink">{t(`${i18nKey}.hoursTitle`)}</h2>
            </div>
            <p className="text-sm leading-relaxed text-muted-foreground">{t(`${i18nKey}.hoursBody`)}</p>
          </section>
          ...
          <section className={cardCls}>
            <div className="mb-2 flex items-center gap-2">
              ...
              <h2 className="font-display text-lg font-bold text-ink">{t(`${i18nKey}.safetyTitle`)}</h2>
            </div>
            <p className="mb-3 text-sm leading-relaxed text-muted-foreground">{t(`${i18nKey}.safetyBody`)}</p>
